...
public class EInventories implements Listener {
	...
	public EInventories(Player player, MCBEnderDragon dragon) {
		this.player = player;
		this.dragon = dragon;
	}
	...
	public void openBarMenu(Player player, MCBEnderDragon dragon) {
		inventory = Bukkit.createInventory(null, InventoryType.HOPPER, "Boss Bar Menu");
	...
	public void openBarColorMenu(Player player, MCBEnderDragon dragon) {
		inventory = Bukkit.createInventory(null, 9, "Boss Bar Color Menu");
	...
	public void openBarStyleMenu(Player player, MCBEnderDragon dragon) {
		inventory = Bukkit.createInventory(null, InventoryType.HOPPER, "Boss Bar Style Menu");
	...
	public void openBarFlagMenu(Player player, MCBEnderDragon dragon) {
		inventory = Bukkit.createInventory(null, InventoryType.HOPPER, "Boss Bar Flag Menu");
	...
	public void onEInventoryInteract(InventoryClickEvent event) {
		...
		if(event.getView().getTitle().equals("Boss Bar Menu")) {
			MCBEnderDragon dragon = new MCBEnderDragon((EnderDragon) Bukkit.getEntity(UUID.fromString(event.getWhoClicked().getInventory().getItemInMainHand().getItemMeta().getLore().get(1).replace("§eUUID: §f", ""))));
			event.setCancelled(true);
			Player player = (Player) event.getWhoClicked();
			int slot = event.getRawSlot();
		...
		if(event.getView().getTitle().equals("Boss Bar Color Menu")) {
			MCBEnderDragon dragon = new MCBEnderDragon((EnderDragon) Bukkit.getEntity(UUID.fromString(event.getWhoClicked().getInventory().getItemInMainHand().getItemMeta().getLore().get(1).replace("§eUUID: §f", ""))));
			event.setCancelled(true);
			int slot = event.getRawSlot();
		...
		if(event.getView().getTitle().equals("Boss Bar Style Menu")) {
			MCBEnderDragon dragon = new MCBEnderDragon((EnderDragon) Bukkit.getEntity(UUID.fromString(event.getWhoClicked().getInventory().getItemInMainHand().getItemMeta().getLore().get(1).replace("§eUUID: §f", ""))));
			event.setCancelled(true);
			int slot = event.getRawSlot();
			if(event.getClickedInventory() == event.getInventory() && event.getInventory().getItem(slot) != null)
